...
internal sealed class AuthenticationService : IAuthenticationService
{
    ...
    {
        var userRequestRepresentationModel = UserRepresentationModel.FromUser( user );

        userRequestRepresentationModel.Credentials = new CredentialRepresentationModel[]
        {
            new()
            {
                Value = password,
        ...
        var response = await _httpClient.PostAsJsonAsync(
            "users",
            userRequestRepresentationModel,
            cancellationToken );
